...
def main(config_path):
    ...
    batch_size_eval = config.get('batch_size_eval', 10)
    ...
    vocoder_path=config.get('vocoder_path',"../../Vocoder/hifigan/checkpoint-2500000steps.pkl")
    vocoder=load_model(vocoder_path).to(device).eval()

    # build model
    model, model_ema = build_model(Munch(config['model_params']), F0_model, ASR_model)
    total = sum([param.nelement() for param in model.generator.parameters()])
    ...
    if config['iswandb']:
        wandb.init(
            project="stream-vc",
            name=f"{osp.basename(log_dir)}/a6000({config['device'].split(':')[-1]})",
            config=config
        )
